...
function App() {
  ...
  const [selectedNode, setSelectedNode] = useState<VRPNode | null>(null);
  ...
  const handleAddNode = useCallback((newNode: VRPNode) => {
   ...
   const handleRemoveNode = useCallback((nodeIdToRemove: number) => {
  ...
  const handleNumVehiclesChange = useCallback((count: number) => {
   ...
   const handleSkillsChange = useCallback((newAvailableSkills: string[], newVehicleSkills: {[key: string]: string[]}) => {
        ...
        <Box sx={{ display: 'flex', height: '100vh', p: 1, gap: 1 }}>
          ...
          >
             ...
             <ControlPanel
                ...
                onSolve={() => { /* Triggered by useMutation */} } // Let react-query handle solve
